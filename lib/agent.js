...
class Agent {
  ...
  async performInference(userMessage, currentContext) {
    ...
    if (this.config.model_vendor === 'openai') {
      ...
      const payload = {
        ...
        max_output_tokens: this.config.max_tokens || 4096,
      ...
      if (currentContext && this.config.tool_options?.use_cached_documents)
        payload.documents = currentContext['__documents__']
    ...
    //Ah, Gemini... 
      ...
      const chat = model.startChat({
        history: this.messages.filter(x => x.role != "system").map(msg => ({ role: role2role[msg.role], parts: [{ text: msg.content }] })),
        generationConfig: {
          temperature: this.config.temperature || 0.7,
    ...
    for await (const chunk of stream) {
      ...
      if (this.config.model_vendor === "cohere") {
        ...
        if (chunk.eventType == "stream-end") {
          try
          {
            ...
            if (this.config.tool_options?.cache_documents && currentContext) 
              currentContext["__documents__"] = chunk.response.documents || null
      ...
      if (writeNextChunkToOutputStream && this.config?.response_format != "json_object")
